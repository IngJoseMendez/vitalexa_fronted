...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    NotificationService.connect((notification) => {
      if (notification.type === 'INVENTORY_UPDATE') {
        console.log("­ƒôª Inventory update received, refreshing data...");
  ...
  return (
    <div className="admin-dashboard">
      ...
      {/* ÔöÇÔöÇ SIDEBAR IZQUIERDA ÔöÇÔöÇ */}
      <aside className="admin-sidebar">
        ...
        <nav className="sidebar-nav">
          {[
            { key: 'orders', icon: 'assignment', label: '├ôrdenes' },
            ...
            { key: 'nomina', icon: 'payments', label: 'N├│mina' },
          ].map(item => (
            <button
              key={item.key}
        ...
        {/* Bot├│n refresh en la parte inferior */}
        <button
...
// ============================================
// PANEL DE ├ôRDENES CON AUTO-ACTUALIZACI├ôN
// ============================================
// ============================================
// PANEL DE ├ôRDENES CON PDF DE FACTURA
// ============================================
function OrdersPanel({ refreshTrigger }) {
  ...
  // ÔöÇÔöÇ PAGINACI├ôN ÔöÇÔöÇ
  const [currentPage, setCurrentPage] = useState(0);
  const [pageSize, setPageSize] = useState(20);
  const [totalPages, setTotalPages] = useState(0);
  const [totalElements, setTotalElements] = useState(0);

  // Ô£à NEW STATE FOR ASSORTMENT
  ...
  // Ô£à STATE FOR COMPLETE ORDER MODAL
  ...
  const fetchOrders = useCallback(async (page, size, status) => {
    ...
    try {
      ...
      console.log(`Ô£à ├ôrdenes actualizadas: p${data.number + 1}/${data.totalPages}, ${data.totalElements} total`);
    } catch (error) {
  ...
  const fetchClientesPorVendedor = useCallback(async (vendedorUsername) => {
    try {
      ...
      if (!vendorId) {
        console.warn('No se encontr├│ ID para el vendedor:', vendedorUsername);
        setClientes([]);
        return;
      }
  ...
  useEffect(() => {
    ...
    // Ô£à LISTENER PARA AUTO-ACTUALIZACI├ôN AL RECIBIR NOTIFICACI├ôN
    const handleNewOrder = () => {
      console.log('­ƒöä Auto-actualizando ├│rdenes...');
  ...
  // ÔÜá´©Å ANULAR directamente v├¡a status=ANULADA requiere OWNER.
  // El Admin puede anular ├│rdenes PENDIENTES usando POST /annul (desde OrderDetailModal).
  ...
  // Ô£à NUEVA FUNCI├ôN: Descargar factura PDF
  const handleDownloadInvoice = async (orderId) => {
    try {
      ...
      console.log('Ô£à PDF descargado correctamente');
    } catch (error) {
  ...
  // Ô£à NUEVA FUNCI├ôN: Vista previa del PDF
  ...
  // Ô£à HANDLER FOR OPENING ASSORTMENT MODAL
  const handleOpenAssortment = async (order, item) => {
    try {
      ...
      if (!promotionId) {
        ...
        toast.error("No se pudo identificar la promoci├│n");
        return;
      }
  ...
  // Filtrar client-side (busq├╝eda, vendedor, cliente) sobre la p├ígina actual
  ...
  const filteredOrders = orders
    ...
    .sort((a, b) => {
      // ÔöÇÔöÇÔöÇ FECHA: el backend ya trae los datos ordenados DESC (m├ís recientes primero).
      // Solo invertimos para "asc" dentro de la p├ígina.  NO re-ordenar en "desc"
      // para no romper la paginaci├│n entre p├íginas.
      if (sortBy === 'fecha') {
        if (sortOrder === 'asc') {
          const getEffectiveDate = (o) =>
            o.estado === 'COMPLETADO' && o.completedAt
              ...
              : new Date(o.fecha);
          return getEffectiveDate(a) - getEffectiveDate(b); // asc = m├ís antiguas primero
        }
        return 0; // desc = mantener el orden que trajo el backend (m├ís recientes primero)
      }
      // ÔöÇÔöÇÔöÇ Otros criterios: sort client-side normal ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
      let valA, valB;
      if (sortBy === 'total') {
  ...
  return (
    <div className="orders-panel">
      <div className="panel-header">
        <h2><span className="material-icons-round" style={{ fontSize: '24px', color: 'var(--primary)', verticalAlign: 'middle' }}>assignment_turned_in</span> Gesti├│n de ├ôrdenes</h2>
      </div>
      ...
      <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
        <div className="sorting-controls">
          ...
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="sort-select">
            ...
            <option value="invoiceNumber">N├║mero de Factura</option>
          </select>
          <button
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem', flexWrap: 'wrap', gap: '0.5rem' }}>
        ...
        <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
          {totalElements > 0 && `${totalElements} ├│rdenes totales`}
        </span>
      </div>
      {/* Banner de advertencia si hay filtros locales activos con paginaci├│n */}
      {(invoiceSearch.trim() || selectedVendedor || selectedCliente) && totalPages > 1 && (
        <div style={{
          ...
          <div>
            <h4 style={{ margin: '0 0 0.25rem 0', color: '#b45309', fontSize: '0.95rem' }}>Atenci├│n con los filtros</h4>
            <p style={{ margin: 0, color: '#92400e', fontSize: '0.85rem', lineHeight: '1.4' }}>
              Los filtros de <strong>b├║squeda, vendedor y cliente</strong> se aplican ├║nicamente a las <strong>{pageSize} ├│rdenes</strong> de la p├ígina actual. Para buscar en toda la base de datos de esta categor├¡a, te sugerimos mostrar m├ís ├│rdenes por p├ígina (ej. 100) o buscar en las p├íginas siguientes.
            </p>
          </div>
        </div>
      )}
              ...
              <div key={order.id} className={`order-card ${order.isSROrder ? 'is-sr' : 'is-normal'} ${paymentStatusClass}`}>
                ...
                {order.items && order.items.length > 0 ? (
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', flex: 1 }}>
                            ...
                            {item.outOfStock && item.estimatedArrivalDate && (
                              <span style={{ fontSize: '0.75rem', color: '#d97706' }}>
                                ­ƒôà ETA: {new Date(item.estimatedArrivalDate).toLocaleDateString('es-ES')}
                  ...
                  <div style={{
                    ...
                    <span>Orden solo con promoci├│n (sin productos regulares)</span>
                  </div>
                )}

                {/* Ô£à NUEVA SECCI├ôN: BOTONES DE FACTURA PDF */}
                <div className="invoice-actions">
                  <h4 style={{ fontSize: '13px', marginBottom: '8px', color: '#6b7280' }}>
                    ­ƒôä Factura / Orden de Empaque
                  </h4>
                ...
                {/* Ô£à SECCI├ôN DE DESCUENTOS - ADMIN */}
                ...
                {/* Ô£à BOTONES DE GESTI├ôN DE ORDEN */}
      ...
      {/* ÔöÇÔöÇ PAGINATION FOOTER ÔöÇÔöÇ */}
      {totalPages > 1 && (
        <div style={{
          display: 'flex',
          alignItems: 'center',
          ...
          <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginLeft: '0.5rem' }}>
            P├ígina {currentPage + 1} de {totalPages}
          </span>
        </div>
      )}
    </div>
...
function AdminNuevaVentaPanel() {
  ...
  const addToCart = (product) => {
    // Ô£à Logic depends on current mode
    if (isBonifiedMode) {
  ...
  const addPromotionToCart = (promotion, qty = 1) => {
    ...
    const newInstances = Array.from({ length: quantity }, (_, i) => ({
      ...
      // Ô£à Capture Special Promotion ID if present
    ...
    if (quantity > 1) {
      ...
      toast.success('Promoci├│n agregada');
    }
  };
  ...
  const handleAssortmentConfirmation = (items) => {
    ...
    items.forEach(item => {
      ...
      if (existingItemIndex >= 0) {
        ...
        newCartItems.push({
          ...
          // Ô£à Pass down specialPromotionId from the selected promotion
    ...
    if (selectedPromotion) {
      const promoInstance = {
        ...
        // Ô£à Capture Special Promotion ID
        specialPromotionId: selectedPromotion.isSpecial ? selectedPromotion.id : null
      };
    ...
    toast.success('Productos de la promoci├│n agregados al carrito');
  };
  ...
  const updateQuantity = (productId, newQuantity, isBonifiedList = false) => {
    // Permite que el input quede vac├¡o temporalmente
    if (newQuantity === '') {
      if (isBonifiedList) {
        setBonifiedCart(bonifiedCart.map(item =>
  ...
  const handleSubmitOrder = async () => {
    // Ô£à ACTUALIZADO: Permite ├│rdenes solo con bonificados
    ...
    try {
      const orderData = {
        ...
        items: [
          ...cart.map(item => ({
            ...
            // Ô£à Include specialPromotionId in item payload
      ...
      toast.success('┬íVenta registrada exitosamente!');
    ...
    <div className="admin-sales-panel">
      ...
      <div className="sales-products-column">
        ...
        <div className="sales-products-list">
          ...
          <h4 style={{ margin: '0.5rem 0 1rem 0', color: 'var(--text-secondary)', fontSize: '0.9rem', fontWeight: 700, textTransform: 'uppercase' }}>Cat├ílogo de Productos</h4>
              ...
              {cart.map(item => (
                <div key={item.productId} className="cart-item">
                  ...
                  <div className="cart-item-qty-control">
                    <button className="btn-qty" onClick={() => updateQuantity(item.productId, item.cantidad - 1, false)}>ÔêÆ</button>
                    <input
                      className="qty-input"
                      type="number"
                      value={item.cantidad}
              ...
              {bonifiedCart.map(item => (
                <div key={item.productId} className="cart-item" style={{ background: '#f0fdf4' }}>
                  ...
                  <div className="cart-item-qty-control" style={{ background: 'white', border: '1px solid #bbf7d0' }}>
                    <button className="btn-qty" onClick={() => updateQuantity(item.productId, item.cantidad - 1, true)}>ÔêÆ</button>
                    <input
                      className="qty-input"
                      type="number"
                      value={item.cantidad}
        ...
        <div style={{
          ...
          <div style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '2px solid #e5e7eb' }}>
            ...
            <div style={{
              ...
              <strong>Nota:</strong> NinaTorres y YicelaSandoval tienen datos unificados. El reporte de cualquiera mostrar├í datos combinados.
            </div>
          </div>
        </div>
      </div>
